...
model = pickle.load(open("model.knn", "rb"))
...
while True:
    ...
    if this_frame:
        ...
        for i in range(len(detected_face)):
            ...
            closest_distance = model.kneighbors([encoded_face], n_neighbors=2)

            unknown = True

            if np.min(closest_distance) <= 0.4:
                unknown = False

            confidence = round(1 - np.min(closest_distance), 2)
